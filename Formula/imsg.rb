# Homebrew formula for imsg — fast, local iMessage CLI + MCP server for macOS.
#
# Builds a Rust extension (imsgcore) from the published sdist via maturin, plus
# a few other Rust-backed wheels (cryptography, pydantic-core, rpds-py) from
# source — so `rust` and `openssl@3` are build-time requirements.
#
# When you cut a new release, bump `url`/`sha256` to the new sdist and refresh
# the resource blocks (brew update-python-resources Formula/imsg.rb).
class Imsg < Formula
  include Language::Python::Virtualenv

  desc "Fast, local iMessage for your terminal + MCP server (macOS only)"
  homepage "https://github.com/ml-lubich/imsg"
  url "https://github.com/ml-lubich/imsg/releases/download/v0.1.0/imsg-0.1.0.tar.gz"
  sha256 "bcd3e1093b8a4e31a3841d76fb262a26e39c2b08192d910fb27c59b59db4a711"
  license "MIT"
  ...
  test do
    assert_match "imsg 0.1.0", shell_output("#{bin}/imsg version")
  end
end
